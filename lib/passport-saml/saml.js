...
SAML.prototype.validateResponse = function (samlResponse, callback) {
  ...
  parser.parseString(xml, function (err, doc) {
    ...
    var response = self.getElement(doc, 'samlp:Response');

    if (response) {
      // Try to parse the saml status message..
      var status = self.getElement(response, 'samlp:Status');
      if (status) {
        var statusCode = self.getElement(status[0], 'samlp:StatusCode');
        if (statusCode) {
          var failed = statusCode[0].$.Value.indexOf('AuthnFailed');
          if (failed) {
            // friendlier than Missing SAML assertion like below..
            return callback(new Error('Authentication Failed'), null, false);
          }
        }
      }
      
      var assertion = self.getElement(response, 'saml:Assertion');
      if (!assertion) {
        return callback(new Error('Missing SAML assertion'), null, false);
      }

      profile = {};
      var issuer = self.getElement(assertion[0], 'saml:Issuer');
      if (issuer) {
        profile.issuer = issuer[0];
      }

      var subject = self.getElement(assertion[0], 'saml:Subject');
      if (subject) {
        var nameID = self.getElement(subject[0], 'saml:NameID');
        if (nameID) {
      ...
      var attributeStatement = self.getElement(assertion[0], 'saml:AttributeStatement');
      if (!attributeStatement) {
        return callback(new Error('Missing AttributeStatement'), null, false);
      }

      attributeStatement.forEach(function (attribute) {
        var attrs = self.getElement(attribute, 'saml:Attribute');
        if (!attrs) return;

        attrs.forEach(function (attr) {
          var value = self.getElement(attr, 'saml:AttributeValue');
          if (typeof value[0] === 'string') {
            profile[attr.$.Name] = value[0];
          } else {
            profile[attr.$.Name] = value[0]._;
          }
        });
      })
